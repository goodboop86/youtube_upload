...
def get_upload_info(client, conf, status):
    ...
    # spreadシートからtsv中身を取得
    header = client.spreadsheets().values().get(spreadsheetId=pconf.context.spread.UPLOAD_LIST_ID,
                                                range=conf.get("HEADER_RANGE")).execute().get('values', [])

    body = client.spreadsheets().values().get(spreadsheetId=pconf.context.spread.UPLOAD_LIST_ID,
                                              range=conf.get("BODY_RANGE")).execute().get('values', [])

    params = get_params_from_sheet_by_date(
        df=pd.DataFrame(body, columns=header[0]),
        _date=datetime.now().strftime("%Y-%m-%d"))[0]

    file, body = create_youtube_request_param(params, conf)
